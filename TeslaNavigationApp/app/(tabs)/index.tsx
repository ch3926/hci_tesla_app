...
// Hardcoding for testing without API
export default function MapScreen() {
  const [currentLocation, setCurrentLocation] = useState(null);
  ...
  useEffect(() => {
    if (currentLocation) {
      // Initialize with hardcoded stations
      const stations = [
        { latitude: 34.0522, longitude: -118.2437 },
        { latitude: 34.0622, longitude: -118.2537 }
      ];
      setChargingStations(stations);
      updateRoute(stations, batteryInfo);
    }
  }, [currentLocation]);
  ...
  const updateRoute = async (stations, battery) => {
    ...
    try {
      // Get hardcoded optimized waypoints
      const optimizedWaypoints = await dpWaypointOptimizer(
        [currentLocation.latitude, currentLocation.longitude],
        [destination.latitude, destination.longitude],
        battery.currentBattery,
      ...
      const polyline = [
        currentLocation,
        { latitude: 34.0522, longitude: -118.2437 }, // Downtown Station
        { latitude: 34.0622, longitude: -118.2537 }, // Westside Station
        destination
      ];
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Update Route" onPress={() => updateRoute(chargingStations, batteryInfo)} />
      <Text>Battery: {batteryInfo.currentBattery}% / {batteryInfo.maxBattery}%</Text>
    </View>
  );
}
